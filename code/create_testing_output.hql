...
CREATE TABLE hdfs_database.million_train_n AS 
select song, count(distinct user) as count_user
from hdfs_database.million_train
group by song; 

## step 4: play count threshold
set play_count_factor=0.1;
CREATE TABLE hdfs_database.million_train_nb_all AS 
...
from (select driver_song_id, also_song_id, play_count from hdfs_database.million_train_nb) as a 
join (select song, count_user from hdfs_database.million_train_n) as b 
on a.also_song_id = b.song
cross join (select count(distinct user) as count_n_users
from hdfs_database.million_train) c 
join (
...
CREATE TABLE hdfs_database.million_train_output AS 
...
from hdfs_database.million_train_nb_all) as a 
where a.rank <= 10;

## step 6: take output from previous and step final output for testing, which matched up predicted to testing set
INSERT OVERWRITE LOCAL DIRECTORY '/home/username/nb_test_results' ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
SELECT b.user, b.song, a.also_song_id, a.play_count, a.adjust_p, a.rank, count(c.song) as predicted_song 
FROM hdfs_database.million_train_output a 
JOIN hdfs_database.million_test_first_half b 
ON a.driver_song_id = b.song 
LEFT JOIN hdfs_database.million_test_second_half c 
ON b.user = c.user  
...
scp -i ~/key_location username@user:/home/username/nb_test_results/nb_test_output ~/UST_GPS/SEIS_734/project/
...
CREATE TABLE hdfs_database.million_train_output_cf AS 
select a.driver_song_id, a.also_song_id, a.both as play_count, a.adjust_p, a.rank 
from (select driver_song_id, also_song_id, both, adjust_p,
row_number() over(partition by driver_song_id order by both desc) as rank 
from hdfs_database.million_train_nb) as a 
where a.rank <= 10;

## step 3: take output from previous and step final output for testing, which matched up predicted to testing set
INSERT OVERWRITE LOCAL DIRECTORY '/home/username/cf_test_results' ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
SELECT b.user, b.song, a.also_song_id, a.play_count, a.adjust_p, a.rank, count(c.song) as predicted_song 
FROM hdfs_database.million_train_output_cf a 
JOIN hdfs_database.million_test_first_half b 
ON a.driver_song_id = b.song 
LEFT JOIN hdfs_database.million_test_second_half c 
ON b.user = c.user  
...
scp -i ~/key_location username@user:/home/username/cf_test_results/cf_test_output ~/UST_GPS/SEIS_734/project/

# AR  ********************************

## move to SAP HANA results to hadoop
scp -i ~/key_location ~/UST_GPS/SEIS_734/project/ar_rules_0504_1.csv username@user:/home/username/ar-training/
make a dir to put the file in:  hadoop fs -mkdir /teamspace/bdpexpp01/ar-training/
hadoop fs -copyFromLocal /home/username/ar-training/ar_rules_0504_1.csv  /teamspace/bdpexpp01/ar-training

## step 1: create table in hive
CREATE EXTERNAL TABLE IF NOT EXISTS hdfs_database.million_train_association ( 
prerule string, 
postrule string,
support float,
confidence float,
...
CREATE TABLE hdfs_database.million_train_output_assocation AS 
...
from hdfs_database.million_train_association) a 
WHERE a.rank <= 10;

## step 3: final output for testing, which matched up predicted to testing set
INSERT OVERWRITE LOCAL DIRECTORY '/home/username/assoc_test_results' ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
SELECT b.user, b.song, a.also_song_id, a.confidence, a.rank, count(c.song) as predicted_song 
FROM hdfs_database.million_train_output_assocation a 
JOIN hdfs_database.million_test_first_half b 
ON a.driver_song_id = b.song 
LEFT JOIN hdfs_database.million_test_second_half c 
ON b.user = c.user  
...
scp -i ~/key_location username@user:/home/username/assoc_test_results/assoc_test_results ~/UST_GPS/SEIS_734/project/
