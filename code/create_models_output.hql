...
CREATE TABLE hdfs_database.million_test_cooccur_v2 AS 
...
join (
select user, song, play_count
from hdfs_database.million_all) b 
on a.user = b.user
where a.song != b.song
...
CREATE TABLE hdfs_database.million_test_cf_results_v2 AS 
...
from hdfs_database.million_test_cooccur_v2) a 
WHERE a.rank <= 10;
...
CREATE TABLE hdfs_database.million_test_n AS 
select song, count(distinct user) as count_user
from hdfs_database.million_all 
group by song; 

## step 3: play count threshold
set play_count_factor=0.1;
CREATE TABLE hdfs_database.million_test_nb_all_v2 AS 
...
from (select driver_song_id, also_song_id, play_count from hdfs_database.million_test_cooccur_v2) as a 
join (select song, count_user from hdfs_database.million_test_n) as b 
on a.also_song_id = b.song
cross join (select count(distinct user) as count_n_users
from hdfs_database.million_all) c 
join (
...
CREATE TABLE hdfs_database.million_test_output_v2 AS 
...
from hdfs_database.million_test_nb_all_v2) as a 
where a.rank <= 10;

# AR  ********************************

## move to SAP HANA results to hadoop
scp -i ~/key_location ~/UST_GPS/SEIS_734/project/song_ar_data_jason.csv username@user:/home/username/
make a dir to put the file in:  hadoop fs -mkdir /teamspace/bdpexpp01/testing_data/association/
hadoop fs -copyFromLocal /home/username/song_ar_data_jason.csv /teamspace/bdpexpp01/testing_data/association

## step 1: create table in hive
CREATE EXTERNAL TABLE IF NOT EXISTS hdfs_database.million_test_association ( 
prerule string, 
postrule string,
support float,
confidence float,
...
CREATE TABLE hdfs_database.million_test_assocation_results AS 
